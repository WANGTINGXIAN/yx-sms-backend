...
public class DefaultParamsAspect {
    ...
    public void beforeEvent(JoinPoint point) {
        ...
        for (Object arg : args) {
            Class<?> classes = arg.getClass();//参数类型
            ...
            if (id == null) {
                ...
                method = getMethod(classes, "setCreatetime", LocalDateTime.class);//设置创建人的方法
                if (method != null) {
                    method.invoke(arg, LocalDateTime.now());
                }
            }
            //1.2 删除 删除人 删除时间
            method = getMethod(classes, "setUpdateUser", String.class);//设是删除人的方法
            if (method != null) {
                method.invoke(arg, userId.toString());
            }
            method = getMethod(classes, "setUpdatetime", LocalDateTime.class);//设置创建人的方法
            if (method != null) {
                method.invoke(arg, LocalDateTime.now());
            }
        }
